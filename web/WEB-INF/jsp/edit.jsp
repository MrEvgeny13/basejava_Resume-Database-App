...
<section>
    <form method="post" action="resume" enctype="application/x-www-form-urlencoded">
        ...
        <h1>Имя:</h1>
        <dl>
            <input type="text" name="fullName" size=55 pattern="[A-Za-zА-Яа-яЁё]{2,16}"
                   title="Please enter your name (only letters, from 2 to 16)" required="required"
        ...
        <c:forEach var="type" items="<%=ContactType.values()%>">
            <dl>
                <dt>${type.title}</dt>
                <dd><input type="text" name="${type.name()}" size=30 pattern=".{1,36}"
                           title="Please enter your contacts up to 36 characters"
                    ...
                    <c:forEach var="org" items="<%=((OrganizationSection) section).getOrganizations()%>"
                        ...
                        <div style="margin-left: 30px">
                            <c:forEach var="pos" items="${org.positions}">
                                <jsp:useBean id="pos" type="com.evgeny13.basejava.model.Organization.Position"/>
                                <dl>
                                    <dt>Начальная дата:</dt>
                                    <dd>
                                        <input type="text" name="${type}${counter.index}startDate" size=10
                                               pattern="[0-12]{1,2}/[0001-9999]{4}"
                                               title="Your date should be in the range from 01/1970 to now"
